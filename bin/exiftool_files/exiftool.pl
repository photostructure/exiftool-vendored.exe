...
# hack by Oliver Betz for "not PAR packed" version: check $^X instead of $0
if ($^X =~ /\(([^\\\/]+)\)(.exe|.pl)?$/i) {
    my $argstr = $1;
  ...
  Option Details
   Tag operations
    ...
    -tagsFromFile *SRCFILE* or *FMT*
         ...
         *SRCFILE* may be the same as *FILE* to move information around
         ...
         only when interpolated in a string.
   ...
   Input-output text formatting
    ...
    -p *FMTFILE* or *STR* (-printFormat)
         ...
         defined as a group of consecutive files with the same section
         header (eg. files are grouped by directory if "#[SECT]" contains
         $directory). Lines beginning with "#[BODY]" and lines not beginning
         with "#" are output for each processed file. Lines beginning with
         "#[IF]" are not output, but all BODY lines are skipped if any tag
         on an IF line doesn't exist. Other lines beginning with "#" are
