  ...
  Option Details
   ...
   Input-output text formatting
    ...
    -lang [*LANG*]
         ...
         *LANG* is "de", "fr", "ja", etc. Use -lang with no other arguments
         ...
         7. Email the module ("ru.pm" in this example) to phil at
         owl.phy.queensu.ca
   ...
   Processing control
    ...
    -ee (-extractEmbedded)
         ...
         videos. See
         <http://owl.phy.queensu.ca/~phil/exiftool/geotag.html#Inverse> for
         examples.
    ...
    -fast[*NUM*]
         Increase speed of extracting information. With -fast, ExifTool will
         not scan to the end of a JPEG image to check for an AFCP or
         PreviewImage trailer, or past the first comment in GIF images or
         the audio/video data in WAV/AVI files to search for additional
         metadata. These speed benefits are small when reading images
         directly from disk, but can be substantial if piping images through
         a network connection. For more substantial speed benefits, -fast2
         ...
         file header, and determines FileType based only on the file
         extension. Has no effect when writing.

         Note that a separate -fast setting may be used for evaluation of a
         -if condition. See the -if option documentation for details.
    ...
    -z (-zip)
         ...
         Compress and Compact="Padding,Shorthand".
   ...
   Special features
    -geotag *TRKFILE*
         ...
         GPSPitch, GPSRoll and AmbientTemperature. By default, tags are
         created in EXIF, and updated in XMP only if they already exist.
         However, "EXIF:Geotime" or "XMP:Geotime" may be specified to write
         only EXIF or XMP tags respectively. Note that GPSPitch and GPSRoll
         are non-standard, and require user-defined tags in order to be
         written.
